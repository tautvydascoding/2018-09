...
function getAboutus($nr) {
$manoSQL = "SELECT * FROM aboutus WHERE id = $nr"; //vietoi id skaiciaus panaudojam kintamaji $nr kad kur iskvestume tas ir butu gydytojas
...
function createAboutus($pname, $paragraph) {
  $manoSQL = "INSERT INTO `aboutus` VALUES(NULL, '$pname', '$paragraph')";
...
function createAboutusIlgesne($pname, $paragraph) {
  //saugumas: uzkuodoja spec simbolius "" '' \n \t <>;
  $pname = mysqli_real_escape_string(getPrisijungimas(), $pname);
  $paragraph = mysqli_real_escape_string(getPrisijungimas(), $paragraph);
...
function deleteAboutus($nr) {
  $nr = mysqli_real_escape_string(getPrisijungimas(), $nr); // kad uzduodot $nr
  $manoSQL = "DELETE FROM `aboutus` WHERE id = '$nr'";
  $arPavyko = mysqli_Query(getPrisijungimas(), $manoSQL);
  if ($arPavyko == false && $arRodytiZinutes == true) { //(!$arPavyko && $arRodytiZinutes) jei parasai if su sauktuku tikrini atbuline reiksme.
    echo "ERROR: nepavyko sukurti gydytojo!!!! <br/>".mysqli_error(getPrisijungimas()); // isvardina net kur klaida yra
  }
...
function editDoctor($nr, $pname, $paragraph) {
  $nr = mysqli_real_escape_string(getPrisijungimas(), $nr);
  $pname = mysqli_real_escape_string(getPrisijungimas(), $pname);
  $paragraph = mysqli_real_escape_string(getPrisijungimas(), $paragraph);
  $manoSQL = "UPDATE `aboutus` SET name = '$pname', lname = '$paragraph' WHERE id = '$nr'";
  mysqli_query(getPrisijungimas(), $manoSQL);
}
...
function getAboutuss($kiekis){
  $kiekis = mysqli_real_escape_string(getPrisijungimas(), $kiekis);
  $manoSQL = "Select * FROM aboutus ORDER BY id DESC LIMIT $kiekis";
