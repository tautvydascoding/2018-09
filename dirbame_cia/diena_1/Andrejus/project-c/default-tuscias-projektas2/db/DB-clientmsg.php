...
// function getClientmesg($nr) {
// $manoSQL = "SELECT * FROM clientmesg WHERE id = $nr"; //vietoi id skaiciaus panaudojam kintamaji $nr kad kur iskvestume tas ir butu gydytojas
...
//
// function createClientmesg($name, $email, $message) {
...
//   }
// }
// // createDoctorIlgesne($_GET['vardas'], $_GET['pastas'], $_GET['klausimas']);
//
// // delete // NOTE:
// //$nr - gydytojo id, kuris bus istrintas is DB
// function deleteClientmesg($nr) {
//   $nr = mysqli_real_escape_string(getPrisijungimas(), $nr); // kad uzduodot $nr
//   $manoSQL = "DELETE FROM `clientmesg` WHERE id = '$nr'";
//   $arPavyko = mysqli_Query(getPrisijungimas(), $manoSQL);
//   if ($arPavyko == false && $arRodytiZinutes == true) { //(!$arPavyko && $arRodytiZinutes) jei parasai if su sauktuku tikrini atbuline reiksme.
//     echo "ERROR: nepavyko sukurti gydytojo!!!! <br/>".mysqli_error(getPrisijungimas()); // isvardina net kur klaida yra
//   }
...
//
// function editClientmesg($nr, $name, $email, $message) {
//   $nr = mysqli_real_escape_string(getPrisijungimas(), $nr);
//   $name = mysqli_real_escape_string(getPrisijungimas(), $name);
//   $email = mysqli_real_escape_string(getPrisijungimas(), $email);
//   $message = mysqli_real_escape_string(getPrisijungimas(), $message);
//   $manoSQL = "UPDATE `clientmesg` SET Name = '$name', Email = '$email', Message = '$message' WHERE id = '$nr'";
//   mysqli_query(getPrisijungimas(), $manoSQL);
// }
...
//
// function getClientmesgs($kiekis){
//   $kiekis = mysqli_real_escape_string(getPrisijungimas(), $kiekis);
//   $manoSQL = "Select * FROM clientmesg ORDER BY id DESC LIMIT $kiekis";
