...
function getNewacc($nr) {
$manoSQL = "SELECT * FROM newacc WHERE id = $nr"; //vietoi id skaiciaus panaudojam kintamaji $nr kad kur iskvestume tas ir butu gydytojas
...
function createNewacc($username, $email, $name, $lname, $password, $timestamp) {
  $manoSQL = "INSERT INTO `newacc` VALUES(NULL, '$username', '$email', '$name', '$lname', '$password', '$timestamp')";
...
function createNewaccIlgesne($username, $email, $name, $lname, $password) {
  ...
  $manoSQL = "INSERT INTO `newacc` VALUES(NULL, '$username', '$email', '$name', '$lname', '$password', NOW())";
...
createNewaccIlgesne($_POST['username'], $_POST['email'], $_POST['name'], $_POST['lname'], $_POST['password']); //veikia!!!
header("session-file.php");

// delete // NOTE:
//$nr - gydytojo id, kuris bus istrintas is DB
function deleteNewacc($nr) {
  $nr = mysqli_real_escape_string(getPrisijungimas(), $nr); // kad uzduodot $nr
  $manoSQL = "DELETE FROM `newacc` WHERE id = '$nr'";
  $arPavyko = mysqli_Query(getPrisijungimas(), $manoSQL);
  if ($arPavyko == false && $arRodytiZinutes == true) { //(!$arPavyko && $arRodytiZinutes) jei parasai if su sauktuku tikrini atbuline reiksme.
    echo "ERROR: nepavyko sukurti gydytojo!!!! <br/>".mysqli_error(getPrisijungimas()); // isvardina net kur klaida yra
  }
...
function editNewacc($nr, $username, $email, $name, $lname, $password, $timestamp) {
  ...
  $lname = mysqli_real_escape_string(getPrisijungimas(), $lname);
  $password = mysqli_real_escape_string(getPrisijungimas(), $password);
  $timestamp = mysqli_real_escape_string(getPrisijungimas(), $timestamp);
  $manoSQL = "UPDATE `newacc` SET UserName = '$username', Email = '$email', Name = '$name', LName = '$lname', Password = '$password', TimeStamp = '$timestamp' WHERE ID = '$nr'";
  mysqli_query(getPrisijungimas(), $manoSQL);
}
...
function getNewaccs($kiekis){
  $kiekis = mysqli_real_escape_string(getPrisijungimas(), $kiekis);
  $manoSQL = "Select * FROM newacc ORDER BY id DESC LIMIT $kiekis";
