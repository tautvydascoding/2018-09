...
function getmtBike($nr) {
$manoSQL = "SELECT * FROM mtbikes WHERE ID = $nr"; //vietoi id skaiciaus panaudojam kintamaji $nr kad kur iskvestume tas ir butu gydytojas
...
function createmtBike($picture, $bname, $lildescrb, $describ, $spec, $price) {
  $manoSQL = "INSERT INTO `mtbikes` VALUES(NULL, '$picture', '$bname', '$lildescrb', '$describ', '$spec', '$price')";
...
function createmtBikeIlgesne($picture, $bname, $lildescrb, $describ, $spec, $price) {
...
function editmtBike($nr, $picture, $bname, $lildescrb, $describ, $spec, $price) {
  ...
  $manoSQL = "UPDATE `mtdoctors` SET Picture = '$picture', Bname = '$bname', LilDescrb = '$lildescrb', Description = '$describ', Specs = '$spec', Price = '$price' WHERE ID = '$nr'";
  mysqli_query(getPrisijungimas(), $manoSQL);
}
// editmtBikes(16, "Onute", "Onetiene");
...
function getmtBikes($kiekis){
  $kiekis = mysqli_real_escape_string(getPrisijungimas(), $kiekis);
  $manoSQL = "Select * FROM mtbikes ORDER BY id DESC LIMIT $kiekis";
